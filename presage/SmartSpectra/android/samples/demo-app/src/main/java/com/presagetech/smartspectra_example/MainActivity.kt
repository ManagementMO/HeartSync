...
// HeartSync: OkHttp for posting biometrics to Flask server
...
class MainActivity : AppCompatActivity() {
    ...
    private var apiKey = "rfXF5tray34Vzni65PeoA5cLpRDVyfTF4g9bdmq8"

    // ============================================================
    //  HEARTSYNC CONFIG — Change these two values before building!
    // ============================================================
    // Set your laptop's local IP (find via ipconfig / ifconfig / ip addr)
    // e.g. "http://192.168.1.42:5000/api/biometrics"
    private val heartSyncServerUrl = "http://100.66.24.125:5000/api/biometrics"
    // Set to "A" for phone #1, "B" for phone #2
    private val heartSyncPersonId = "A"
    // OkHttp client for posting to HeartSync server
    ...
    // ============================================================
    //  HEARTSYNC: Post latest biometrics to Flask server
    // ============================================================
    private fun postBiometricsToHeartSync(heartRate: Float, breathingRate: Float) {
        Thread {
            try {
                val json = JSONObject().apply {
                    put("person_id", heartSyncPersonId)
                    put("heart_rate", heartRate.toDouble())
                    put("breathing_rate", breathingRate.toDouble())
                    put("timestamp", System.currentTimeMillis())
                }
                val body = json.toString().toRequestBody(jsonMediaType)
                val request = Request.Builder()
                    .url(heartSyncServerUrl)
                    .post(body)
                    .build()
                httpClient.newCall(request).execute().use { response ->
                    Log.d("HeartSync", "Posted HR=$heartRate BR=$breathingRate -> ${response.code}")
                }
            } catch (e: Exception) {
                Log.w("HeartSync", "Failed to post biometrics: ${e.message}")
            }
        }.start()
    }

    private fun handleMetricsBuffer(metrics: MetricsBuffer) {
        // ---- HeartSync: extract latest HR and BR and post to server ----
        val pulse = metrics.pulse
        val breathing = metrics.breathing
        val latestHr = if (pulse.rateCount > 0) pulse.rateList.last().value else 0f
        val latestBr = if (breathing.rateCount > 0) breathing.rateList.last().value else 0f
        if (latestHr > 0f) {
            postBiometricsToHeartSync(latestHr, latestBr)
        }
        // ---- End HeartSync ----
